...
static void ImGui_Impl_UpdateMousePos()
{
    ImGuiIO& io = ImGui::GetIO();
...
void ImGui_Impl_NewFrame()
{
    ...
    ImGuiIO& io = ImGui::GetIO();
    IM_ASSERT(io.Fonts->IsBuilt() && "Font atlas not built! It is generally built by the renderer back-end. Missing call to renderer _NewFrame() function? e.g. ImGui_Impl_NewFrame().");
...
IMGUI_IMPL_API uintptr_t ImGui_Impl_WndProcHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CHAR:
        ...
        return 0;
    case WM_SETCURSOR:
        if (LOWORD(lParam) == HTCLIENT && ImGui_Impl_UpdateMouseCursor())
            return 1;
        return 0;
    }
    return 0;
